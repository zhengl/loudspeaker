...
describe("PaintingGestureDetector", function(){
	...
	it("triggers DRAW_TO after MOUSE_DOWN, MOVE_TO and MOVE_TO", function(){
		detector.detect(createEvent(Event.Kinetic.MOUSE_DOWN, 10, 10));
		detector.detect(createEvent(Event.Kinetic.MOVE_TO, 20, 20));
		detector.detect(createEvent(Event.Kinetic.MOVE_TO, 30, 30));

		expect(eventBus.publish).toHaveBeenCalledWith(new Event(Event.Page.DRAW_TO, [{x: 30, y: 30}]));
	});	

	it("triggers FINISH_DRAWING after MOUSE_DOWN, MOVE_TO and MOUSE_UP", function(){
		detector.detect(createEvent(Event.Kinetic.MOUSE_DOWN, 10, 10));
		detector.detect(createEvent(Event.Kinetic.MOVE_TO, 20, 20));
		detector.detect(createEvent(Event.Kinetic.MOUSE_UP, 30, 30));

		expect(eventBus.publish).toHaveBeenCalledWith(new Event(Event.Page.FINISH_DRAWING, [{x: 30, y: 30}]));
	});
	...
	it("triggers START_DRAWING after MOUSE_DOWN, MOVE_TO, MOUSE_UP and MOUSE_DOWN", function(){
		detector.detect(createEvent(Event.Kinetic.MOUSE_DOWN, 10, 10));
		detector.detect(createEvent(Event.Kinetic.MOVE_TO, 20, 20));
		detector.detect(createEvent(Event.Kinetic.MOUSE_UP, 30, 30));
